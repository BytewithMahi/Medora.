...
export default function CustomerDashboard({ onScanVerify }: CustomerDashboardProps) {
    ...
    const handleSearch = async (e?: React.FormEvent) => {
        ...
        const allQuestionsChecked = questions.batchCorrect && questions.sealIntact && questions.expiryLegible;
        
        if (!allQuestionsChecked) {
            // "Answers do not match" -> Verification Failed
            setTimeout(() => {
                            ...
                            >
                                ...
                                <div className="space-y-3">
                                    {[
                                        ...
                                        <label key={q.id} className={`flex items-start gap-4 p-4 rounded-xl border cursor-pointer transition-all ${questions[q.id as keyof typeof questions] ? 'bg-emerald-500/10 border-emerald-500/50' : 'bg-black/30 border-white/5 hover:border-white/20'}`}>
                                            <div className="mt-0.5">
                                                <input
                                                    type="checkbox"
                                                    checked={questions[q.id as keyof typeof questions]}
                                                    onChange={(e) => setQuestions({ ...questions, [q.id]: e.target.checked })}
                                                    className="w-5 h-5 rounded border-white/20 text-emerald-500 focus:ring-emerald-500 focus:ring-offset-0 bg-black/50"
                                                />
                                            </div>
